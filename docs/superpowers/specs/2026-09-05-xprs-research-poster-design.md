...
- Add optional `poster` and `posterAlt` fields to the project content schema and
  set them only on the XPRS entry. This keeps the reusable `ProjectCard`
  component data-driven rather than coupling it to the project title.
...
- Use the poster's descriptive alt text from project content and make the link's
  accessible name explicitly identify the full-size action. No modal,
  client-side state, or new dependency is required.

## Component Changes

1. `src/content.config.ts` accepts optional `poster` and `posterAlt` values.
